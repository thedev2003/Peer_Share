...
export default function MarketplacePage() {
	...
	// Update a product in state (for queue, sell, etc.)
	const updateProductState = (id, updatedProduct) => {
		setProducts(prev =>
			prev.map(p => p._id === id ? updatedProduct : p)
		);
	};

	// Remove a product from marketplace (after sale or removal)
	const removeFromMarketplace = (id) => {
		setProducts(prev =>
			prev.filter(p => p._id !== id)
		);
	};

	// Add product (after submitting new item)
	const handleProductAdded = (newProduct) => {
		setProducts(prev => [newProduct, ...prev]);
	};

...
export default function ProductCard({ product, onProductRemoved }) {
  ...
  const isInQueue = user && interestedBuyers.includes(user._id);
  const isSeller = user && user._id === (seller._id || seller);
  const canChat = isSeller || isInQueue;
  ...
  const handleJoinQueue = async () => {
    ...
    setQueueError(null);
    try {
      await axios.post(`${API_URL}/api/products/${_id}/interested`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setJoined(true);
      setQueueError(null);
    } catch (err) {
      setQueueError(err.response?.data?.message || 'Failed to join queue');
    } finally {
      setJoining(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-xs sm:w-72 rounded-xl overflow-hidden group shadow-lg bg-gray-800 border border-gray-700 mx-auto sm:mx-2 my-4">
      ...
      <div className="p-3">
        ...
        {/* Display errors */}
        {queueError && <div className="text-red-500 text-xs mb-2">{queueError}</div>}
        {removeError && <div className="text-red-500 text-xs mb-2">{removeError}</div>}
        <div className="flex flex-col sm:flex-row gap-2 justify-between mt-2">
          {isSeller && (
            <button
              className="flex-1 px-2 py-2 rounded bg-red-600 text-white text-sm font-semibold hover:bg-red-700"
